...
sources: ["raw/micrograd/engine.ipynb"]
---
...
`trace()`로 그래프의 모든 노드/엣지를 모으고, `draw_dot()`으로 각 노드를 `{ label | data | grad }` 형태로 그려서 계산 흐름을 한눈에 볼 수 있다. 위 예시 그래프를 그려보면 다음과 같다(순전파 방향):
...
`L = d * f` 라는 최종 출력에서 시작해서, **체인룰(chain rule)**을 이용해 각 변수가 `L`에 얼마나 영향을 주는지(`grad`)를 거꾸로 계산한다. 위 그래프에서 grad가 거꾸로 흐르는 방향은 다음과 같다:
